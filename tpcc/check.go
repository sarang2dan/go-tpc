...
func (w *Workloader) checkCondition1(ctx context.Context, warehouse int) error {
	...
	rows, err := s.Conn.QueryContext(ctx, convertToPQ(query, w.cfg.Driver.GetProtocolType()), warehouse)
	if err != nil {
...
func (w *Workloader) checkCondition2(ctx context.Context, warehouse int) error {
	...
	rows, err := s.Conn.QueryContext(ctx, convertToPQ(query, w.cfg.Driver.GetProtocolType()), warehouse, warehouse, warehouse)
	if err != nil {
		return fmt.Errorf("exec %s failed %v", query, err)
	}
	defer rows.Close()
...
func (w *Workloader) checkCondition4(ctx context.Context, warehouse int) error {
	...
	rows, err := s.Conn.QueryContext(ctx, convertToPQ(query, w.cfg.Driver.GetProtocolType()), warehouse, warehouse)
	if err != nil {
		return fmt.Errorf("exec %s failed %v", query, err)
	}
	defer rows.Close()
